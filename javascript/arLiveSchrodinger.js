...
updateServer = (pose, image, eigenvalue) => fetch('thecomputeServer.ca',{
        method: 'POST',
        body: xwwwform({
            'pose':pose,
            'image':image,
    ...
    .then(x=>x.json())
    .then(x=>{
        let [eigs, spectra] = JSON.parse(x)
        updateWavefunction(spectra)
        eigenvalues = eigs
		...
		for (const view of pose.views) {
            ...
            if (captureNext) {
                ...
                if(depthData){
                    ...
                    const currentPosition = JSON.stringify([camera.position, camera.quaternion, depthData.rawValueToMeters, depthData.width, depthData.height])
                    updateServer(currentPosition, jsonString, currentEigenvalue)
                }
            } else {
